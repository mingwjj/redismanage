...
public class RedisUtil {
	...
	// Redis服务器IP
		...
		public static void init() {
			try {
				ADDR = PropertiesUtil.getConfFileText("redis.default.addr");
				PORT = Integer.parseInt(PropertiesUtil.getConfFileText("redis.default.port"));
				AUTH = PropertiesUtil.getConfFileText("redis.default.auth");
				MAX_ACTIVE = Integer.parseInt(PropertiesUtil.getConfFileText("redis.default.maxactive"));
				MAX_IDLE = Integer.parseInt(PropertiesUtil.getConfFileText("redis.default.maxidle"));
				MAX_WAIT = Integer.parseInt(PropertiesUtil.getConfFileText("redis.default.maxwait"));
				TIMEOUT = Integer.parseInt(PropertiesUtil.getConfFileText("redis.default.timeout"));
				TEST_ON_BORROW = Boolean.parseBoolean(PropertiesUtil.getConfFileText("redis.default.testonborrow"));
				...
				try {
					...
					log.info("初始化Redis连接池出错，错误信息" + e.getMessage());
				}
			} catch (Exception e) {
				log.error("初始化Redis连接池出错，错误信息：" + e.getMessage());
			}
		}
     ...
     * @return
     */
    public static Map<String, String> hsearchf(String key, String field){
        Jedis jedis = null;
        Map<String, String> res = null;
        try {
            jedis = pool.getResource();
     ...
     * @return
     */
    public static Map<String, String> hsearchv(String key, String value){
        Jedis jedis = null;
        Map<String, String> res = null;
        try {
            jedis = pool.getResource();
            res = jedis.hgetAll(key);
            // 过滤掉不包含的值
            Iterator<String> it = res.values().iterator();
            while (it.hasNext()) {
            	String v = it.next();
            	if (!v.contains(value)) {
            		it.remove();
     ...
     * @return
     */
    public static Map<String, String> hsearchfv(String key, String field, String value){
        Jedis jedis = null;
        Map<String, String> res = null;
        try {
            jedis = pool.getResource();
            ...
            Iterator<String> it2 = res.values().iterator();
            while (it2.hasNext()) {
            	String v = it2.next();
            	if (!v.contains(value)) {
            		it2.remove();
